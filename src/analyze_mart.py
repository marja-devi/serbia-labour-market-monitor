...
def aggregate_2025_by_field(
    rankings: list[dict[str, object]], group_field: str, output_field: str
    ...
    for row in rankings:
        ...
        if not group_name:
            continue
        grouped[(group_name, str(row["earnings_type"]))].append(float(row["avg_2025_value_rsd"]))

    summary_rows: list[dict[str, object]] = []

    for (group_name, earnings_type), values in sorted(grouped.items()):
        summary_rows.append(
            {
                output_field: group_name,
                "earnings_type": earnings_type,
                "municipality_count": len(values),
                "median_2025_value_rsd": round(median(values), 2),
...
def build_belgrade_novi_sad_trend(validation_rows: list[dict[str, str]]) -> list[dict[str, object]]:
    belgrade_grouped: dict[str, dict[str, list[float]]] = defaultdict(lambda: defaultdict(list))
    novi_sad_grouped: dict[str, dict[str, list[float]]] = defaultdict(lambda: defaultdict(list))
    ...
    for row in validation_rows:
        ...
        if row["city_group_name"] == "Grad Beograd" and row["local_unit_type"] == "city_municipality":
            belgrade_grouped[year][earnings_type].append(value)

        if row["city_group_name"] == "Grad Novi Sad":
            novi_sad_grouped[year][earnings_type].append(value)
    ...
    for year in sorted(set(belgrade_grouped) | set(novi_sad_grouped), key=int):
        ...
        if "net" in belgrade_values and "gross" in belgrade_values:
            net_value = median(belgrade_values["net"])
            gross_value = median(belgrade_values["gross"])
            trend_rows.append(
                {
                    "city_label": "Grad Beograd",
                    "series_method": "median_of_city_municipalities",
                    "year": int(year),
                    "net_value_rsd": round(net_value, 2),
                    "gross_value_rsd": round(gross_value, 2),
        ...
        if "net" in novi_sad_values and "gross" in novi_sad_values:
            net_value = median(novi_sad_values["net"])
            gross_value = median(novi_sad_values["gross"])
            trend_rows.append(
                {
                    "city_label": "Grad Novi Sad",
                    "series_method": "median_of_city_group_members",
                    "year": int(year),
                    "net_value_rsd": round(net_value, 2),
                    "gross_value_rsd": round(gross_value, 2),
                    "gross_minus_net_rsd": round(gross_value - net_value, 2),
...
def write_findings_doc(
    ...
    macro_net_top = top_n(macro_rows, "net", "median_2025_value_rsd", True, n=2)
    district_net_top = top_n(district_rows, "net", "median_2025_value_rsd", True)
    ...
    for row in macro_net_top:
        lines.append(f"- {row['macro_region_name']}: {row['median_2025_value_rsd']} RSD")
    ...
    for row in district_net_top:
        lines.append(f"- {row['administrative_district_name']}: {row['median_2025_value_rsd']} RSD")
...
def main() -> None:
    ...
    macro_rows = aggregate_2025_by_field(rankings, "macro_region_name", "macro_region_name")
    district_rows = aggregate_2025_by_field(rankings, "administrative_district_name", "administrative_district_name")
    city_group_rows = aggregate_2025_by_field(rankings, "city_group_name", "city_group_name")
    ...
    belgrade_novi_sad_rows = build_belgrade_novi_sad_trend(validation_rows)
